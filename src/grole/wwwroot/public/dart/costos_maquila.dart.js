...
["costos_maquila.dart", "costos_maquila.dart", , X, {
facturar: function() {
  if (window.confirm("\u00bfFacturar Ordenes?") === true) {
    H.interceptedTypeCast(document.querySelector("#btnFacturar"), "$isButtonElement").disabled = true;
    var pMapData = P.LinkedHashMap_LinkedHashMap(null, null, null, null, null);
    pMapData.$indexSet(pMapData, "kilos", $.pTotalKilos);
    pMapData.$indexSet(pMapData, "importe", $.pTotalImporte);
    pMapData.$indexSet(pMapData, "clase", H.Primitives_parseInt(H.interceptedTypeCast(document.querySelector("#inpDepto"), "$isSelectElement").value, null, null));
    pMapData.$indexSet(pMapData, "lista", $.pConsulta);
    W.HttpRequest_request("/facturar", "POST", null, new X.facturar_closure(), null, null, P.stringify(pMapData, new P.JsonEncoder(null)._toEncodableFunction), null).then$1(new X.facturar_closure0());
  }
},

llenaGrid: function() {
  var pFolio, pFechaIni, pFechaFin, pClase;
  pFolio = H.Primitives_parseInt(J.get$value$x(H.interceptedTypeCast(document.querySelector("#folio"), "$isHiddenInputElement")), null, null);
  pFechaIni = H.interceptedTypeCast(document.querySelector("#inpFechaIni"), "$isInputElement").value;
  pFechaFin = H.interceptedTypeCast(document.querySelector("#inpFechaFin"), "$isInputElement").value;
  pClase = H.interceptedTypeCast(document.querySelector("#inpDepto"), "$isSelectElement").value;
  W.HttpRequest_getString(J.$eq(pFolio, 0) ? "/consulta_costos_maquila?fecha_ini=" + pFechaIni + "&fecha_fin=" + pFechaFin + "&clase=" + pClase : "/rest/facturas/" + H.S(pFolio), null, null).then$1(new X.llenaGrid_closure(pFolio));
},

inicializa: function() {
  var pFolio, nf, t1, btnConsultar, t2, t3, t4, btnCancelar;
  pFolio = H.Primitives_parseInt(J.get$value$x(H.interceptedTypeCast(document.querySelector("#folio"), "$isHiddenInputElement")), null, null);
  ...
  nf.addPattern$1("dd/MM/yyyy");
  H.interceptedTypeCast(document.querySelector("#inpFechaIni"), "$isInputElement").value = nf.format$1(nf, P.DateTime$_now());
  H.interceptedTypeCast(document.querySelector("#inpFechaFin"), "$isInputElement").value = nf.format$1(nf, P.DateTime$_now());
  t1 = J.getInterceptor$n(pFolio);
  if (t1.$gt(pFolio, 0)) {
    H.interceptedTypeCast(document.querySelector("#inpFechaIni"), "$isInputElement").disabled = true;
    H.interceptedTypeCast(document.querySelector("#inpFechaFin"), "$isInputElement").disabled = true;
    H.interceptedTypeCast(document.querySelector("#inpDepto"), "$isSelectElement").disabled = true;
    H.interceptedTypeCast(document.querySelector("#inpFechaIni"), "$isInputElement").value = "";
    H.interceptedTypeCast(document.querySelector("#inpFechaFin"), "$isInputElement").value = "";
  }
  W.HttpRequest_getString("/rest/clase_ordenp_obtener_lista", null, null).then$1(new X.inicializa_closure());
  btnConsultar = H.interceptedTypeCast(document.querySelector("#btnConsultar"), "$isButtonElement");
  btnConsultar.toString;
  t2 = C.EventStreamProvider_click._eventType;
  t3 = new W._ElementEventStreamImpl(btnConsultar, t2, false);
  H.setRuntimeTypeInfo(t3, [null]);
  t4 = new W._EventStreamSubscription(0, t3._target, t3._eventType, W._wrapZone(new X.inicializa_closure0()), t3._useCapture);
  H.setRuntimeTypeInfo(t4, [H.getRuntimeTypeArgument(t3, "_EventStream", 0)]);
  t4._tryResume$0();
  btnCancelar = H.interceptedTypeCast(document.querySelector("#btnCancelar"), "$isButtonElement");
  btnCancelar.toString;
  t2 = new W._ElementEventStreamImpl(btnCancelar, t2, false);
  H.setRuntimeTypeInfo(t2, [null]);
  t4 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(new X.inicializa_closure1()), t2._useCapture);
...
facturar_closure: {"": "Closure;",
  call$1: function(x) {
  }
},

facturar_closure0: {"": "Closure;",
  call$1: function(ARequest) {
    var pResult = P._parseJson(J.get$responseText$x(ARequest), new P.JsonDecoder(null)._reviver);
    window.alert("Se creo factura folio " + H.S(J.$index$asx(pResult, "id")));
    J.set$href$x(C.Window_methods.get$location(window), "/facturas");
  }
},

llenaGrid_closure: {"": "Closure;pFolio_0",
  call$1: function(Data) {
    var tableLista, nf, df, t1, t2, item, tr, t3, d, td, t4, t5, btnPreFacturar;
    $.pConsulta = P._parseJson(Data, new P.JsonDecoder(null)._reviver);
    tableLista = document.querySelector("#gridLista");
    nf = T.NumberFormat_NumberFormat("###,##0.00", null);
    ...
    df.addPattern$1("dd/MM/yyyy");
    $.pTotalKilos = 0;
    $.pTotalImporte = 0;
    for (t1 = J.get$iterator$ax($.pConsulta), t2 = J.getInterceptor$x(tableLista); t1.moveNext$0();) {
      item = t1.get$current();
      tr = document.createElement("tr", null);
      tr.id = "tr" + C.JSInt_methods.toString$0($.pId);
      t3 = J.getInterceptor$asx(item);
      d = P.DateTime_parse(J.substring$2$s(H.stringTypeCast(t3.$index(item, "fecha")), 0, 10));
      td = document.createElement("td", null);
      td.textContent = df.format$1(df, d);
      t4 = J.getInterceptor$x(tr);
      t5 = t4.get$children(tr);
      t5.add$1(t5, td);
      td = document.createElement("td", null);
      td.textContent = J.toString$0(t3.$index(item, "id_orden"));
      t5 = t4.get$children(tr);
      t5.add$1(t5, td);
      td = document.createElement("td", null);
      td.textContent = t3.$index(item, "producto");
      t5 = t4.get$children(tr);
      t5.add$1(t5, td);
      td = document.createElement("td", null);
      td.textContent = t3.$index(item, "codigosap");
      t5 = t4.get$children(tr);
      t5.add$1(t5, td);
      td = document.createElement("td", null);
      td.textContent = t3.$index(item, "descripcion");
      t5 = t4.get$children(tr);
      t5.add$1(t5, td);
      td = document.createElement("td", null);
      td.textContent = nf.format$1(nf, J.toDouble$0$n(t3.$index(item, "costo_maquila")));
      t5 = J.get$classes$x(td);
      t5.add$1(t5, "lr");
      t5 = t4.get$children(tr);
      t5.add$1(t5, td);
      td = document.createElement("td", null);
      td.textContent = nf.format$1(nf, J.toDouble$0$n(t3.$index(item, "producido")));
      t5 = J.get$classes$x(td);
      t5.add$1(t5, "lr");
      t5 = t4.get$children(tr);
      t5.add$1(t5, td);
      td = document.createElement("td", null);
      td.textContent = nf.format$1(nf, J.toDouble$0$n(t3.$index(item, "importe_maquila")));
      t5 = J.get$classes$x(td);
      t5.add$1(t5, "lr");
      ...
      $.pTotalKilos = $.pTotalKilos + J.toDouble$0$n(t3.$index(item, "producido"));
      $.pTotalImporte = $.pTotalImporte + J.toDouble$0$n(t3.$index(item, "importe_maquila"));
      $.pId = $.pId + 1;
    }
    if (J.$gt$n(J.get$length$asx($.pConsulta), 0)) {
      tr = document.createElement("tr", null);
      tr.id = "tr" + C.JSInt_methods.toString$0($.pId);
      td = document.createElement("td", null);
      td.textContent = "";
      if (J.$eq(this.pFolio_0, 0)) {
        btnPreFacturar = document.createElement("button", null);
        t1 = J.get$classes$x(btnPreFacturar);
        t1.add$1(t1, "btn");
        btnPreFacturar.textContent = "Facturar";
        btnPreFacturar.id = "btnFacturar";
        btnPreFacturar.toString;
        t1 = new W._ElementEventStreamImpl(btnPreFacturar, C.EventStreamProvider_click._eventType, false);
        H.setRuntimeTypeInfo(t1, [null]);
        t3 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new X.llenaGrid__closure()), t1._useCapture);
        H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
        t3._tryResume$0();
        t3 = J.get$children$x(td);
        t3.add$1(t3, btnPreFacturar);
      }
      t1 = J.getInterceptor$x(tr);
      t3 = t1.get$children(tr);
      t3.add$1(t3, td);
      td = document.createElement("td", null);
      td.textContent = "";
      t3 = t1.get$children(tr);
      ...
      t3.add$1(t3, td);
      td = document.createElement("td", null);
      td.textContent = nf.format$1(nf, $.pTotalKilos);
      t3 = J.getInterceptor$x(td);
      t4 = t3.get$classes(td);
      t4.add$1(t4, "lr");
      ...
      t3.add$1(t3, td);
      td = document.createElement("td", null);
      td.textContent = nf.format$1(nf, $.pTotalImporte);
      t3 = J.getInterceptor$x(td);
      t4 = t3.get$classes(td);
      t4.add$1(t4, "lr");
      t3 = t3.get$classes(td);
      ...
      t1.add$1(t1, td);
      t2 = t2.get$tFoot(tableLista);
      new W._ChildrenElementList(t2, t2.children)._element.appendChild(tr);
    }
  }
},

llenaGrid__closure: {"": "Closure;",
  call$1: function(ev) {
    X.facturar();
  }
},
...
inicializa_closure: {"": "Closure;",
  call$1: function(Data) {
    var pDeptos, t1, t2, item, t3;
    pDeptos = H.interceptedTypeCast(document.querySelector("#inpDepto"), "$isSelectElement");
    for (t1 = J.get$iterator$ax(P._parseJson(Data, new P.JsonDecoder(null)._reviver)), t2 = J.getInterceptor$x(pDeptos); t1.moveNext$0();) {
      item = t1.get$current();
      t3 = J.getInterceptor$asx(item);
      t2.insertAdjacentHtml$2(pDeptos, "beforeend", "<option value=\"" + H.S(t3.$index(item, "id")) + "\">" + H.S(t3.$index(item, "clase")) + "</option>");
    }
  }
},
...
DateTime_parse: function(formattedString) {
  ...
  if (match != null) {
    ...
    if (7 >= t2.length)
      throw H.ioore(t2, 7);
    t1 = new P.DateTime_parse_parseDoubleOrZero().call$1(t2[7]);
    if (typeof t1 !== "number")
      return P.DateTime_parse$bailout(1, years, month, day, hour, minute, second, t2, t1);
    ...
    t1 = t2[8];
    if (typeof t1 !== "string")
      return P.DateTime_parse$bailout(2, years, month, day, hour, minute, second, 0, t1, addOneMillisecond, millisecond);
    isUtc = t1 !== "";
...
DateTime_parse$bailout: function(state0, years, month, day, hour, minute, second, t2, t1, addOneMillisecond, millisecond) {
  switch (state0) {
    case 0:
      match = new H.JSSyntaxRegExp(H.JSSyntaxRegExp_makeNative("^([+-]?\\d?\\d\\d\\d\\d)-?(\\d\\d)-?(\\d\\d)(?:[ T](\\d\\d)(?::?(\\d\\d)(?::?(\\d\\d)(.\\d{1,6})?)?)? ?([zZ])?)?$", false, true, false), null, null).firstMatch$1(formattedString);
    default:
      ...
      if (state0 === 2 || state0 === 1 || state0 === 0 && match != null)
        switch (state0) {
          ...
          case 1:
            state0 = 0;
            millisecond = J.round$0$n(J.$mul$n(t1, 1000));
            if (millisecond === 1000) {
              addOneMillisecond = true;
              millisecond = 999;
            } else
...
ButtonElement: {"": "HtmlElement;name=,value=", $isButtonElement: true, "%": "HTMLButtonElement"},
...
InputElement: {"": "HtmlElement;name=,value=", $isInputElement: true, $isElement: true, $isNode: true, $isHiddenInputElement: true, "%": "HTMLInputElement"},
...
TableElement: {"": "HtmlElement;tFoot=",
...
// Native classes
J.Interceptor["%"] = "DOMError|DOMImplementation|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedNumberList";
...
//# sourceMappingURL=costos_maquila.dart.js.map
//@ sourceMappingURL=costos_maquila.dart.js.map
